...
typedef struct AddAnyValueAggregatesContext
{
	...
	/* true if there are expressions in the GROUP BY target entries */
...
BuildJobQuery(MultiNode *multiNode, List *dependentJobList)
{
	...
	{
		targetList = (List *) WrapUngroupedVarsInAnyValueAggregate((Node *) targetList,
																   groupClauseList,
																   targetList);

		havingQual = WrapUngroupedVarsInAnyValueAggregate((Node *) havingQual,
